...
<template>
  <div class="w-full relative z-1" v-for="el in listData" :key="el.paneName">
    <slot :el="el">
      <p
        class="flex justify-items-start w-full text-2xl font-600 color-[#2c3e50]"
        :class="{ 'my-6': el.paneName, 'mt-6': !el.paneName }"
      >
        {{ el.paneName }}
      </p>
      <div class="w-full">
        <el-row :gutter="20">
          <el-col class="mb-4" :xs="24" :sm="8" v-for="item in el.children" :key="item.id">
            <div
              ...
              <a :href="item.href" target="_blank">
                <div class="bg-[#fbfbfb]">
                  ...
                  <div class="flex justify-items-start items-end text-xs p-1">
                    <span v-for="items in item.source" :key="items.id">
